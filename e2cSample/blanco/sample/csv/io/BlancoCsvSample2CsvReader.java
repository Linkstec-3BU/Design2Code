...
/**
 * ファイル定義[BlancoCsvSample2/このクラスは単にサンプルです。タブ区切りのタイトル行無しのサンプルです。]のリーダクラス。
 *
...
public class BlancoCsvSample2CsvReader {
    ...
    public BlancoCsvSample2CsvRecord readRecord() throws BlancoCsvIOException, IOException {
        if (fReader == null) {
            throw new IllegalArgumentException("[BlancoCsvSample2] リーダが設定されていない状態でメソッド[readRecord]が呼び出されました。これは許可されません。リーダをセットしてから呼びだしてください。");
        }
