...
/**
 * ファイル定義[BlancoCsvSample2/このクラスは単にサンプルです。タブ区切りのタイトル行無しのサンプルです。]のライタクラス。
 *
 * このクラスはblancoCsvファイル定義書から自動生成されたライタクラスです。<br>
 * 　1.文字列長チェックには、[EUC_JP]エンコーディングを利用します。<br>
 * 　2.クラスの利用した後は必ず close()を呼び出してください。<br>
 */
public class BlancoCsvSample2CsvWriter {
    /**
     * ライタオブジェクト。
     *
     ...
     * CSVライタクラスのコンストラクタ。
     */
    public BlancoCsvSample2CsvWriter() {
        init();
    }
     ...
     * @param arg 関連づけるライタ。
     */
    public BlancoCsvSample2CsvWriter(final BufferedWriter arg) {
        fWriter = arg;
        init();
    }
    ...
    public void writeTitle() throws IOException {
        if (fWriter == null) {
            throw new IllegalArgumentException("[BlancoCsvSample2] ライタが設定されていない状態でメソッド[writeTitle]が呼び出されました。これは許可されません。ライタをセットしてから呼びだしてください。");
        }
     ...
     * @throws IOException 関連づけられたライタ出力の際に例外が発生した場合。
     */
    public void writeRecord(final BlancoCsvSample2CsvRecord record) throws BlancoCsvIOException, IOException {
        if (fWriter == null) {
            throw new IllegalArgumentException("[BlancoCsvSample2] ライタが設定されていない状態でメソッド[writeRecord]が呼び出されました。これは許可されません。ライタをセットしてから呼びだしてください。");
        }
        ...
        if (writeToken.length() > 0) {
            if (writeToken.getBytes("EUC_JP").length < 1) {
            ...
            if (writeToken.getBytes("EUC_JP").length > 10) {
        ...
        if (writeToken.length() > 0) {
            if (writeToken.getBytes("EUC_JP").length < 4) {
        ...
        if (record.getField3() == null) {
            ...
            if (writeToken.getBytes("EUC_JP").length > 3) {
